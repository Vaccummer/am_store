...
{
    public class Worker // 类定义
    {
        public static byte[] Extract(string exePath, int? index = 0)
        {
            try
            {
                ...
                var icons = IconInfo.LoadIconsFromBinary(exePath, index);
                var icon256 = icons.FirstOrDefault(i => i.WithIcon(icon => icon.Size.Width == 256));

                if (icon256 == null)
                    throw new Exception("No 256x256 icon found!");
                ...
                if (imageData == null)
                {
                    throw new Exception("Icon extraction failed: Icon not loaded.");
                }
                return imageData;
            }
            catch (Exception ex)
            {
                throw new Exception($"Icon extraction failed: {ex.Message}");
            }
        }
    }
    ...
    {
        public IconInfo(int groupIndex, string groupId, int index, string id, IntPtr handle)
        {
    ...
    public class IconExtractor2
    {
        public static void haha(string exePath)
        {
            List<Icon> icons = GetIcons2(exePath);
            Console.WriteLine($"Total icon groups: {icons.Count}");

        }
        public static List<Icon> GetIcons2(string filePath)
        {
