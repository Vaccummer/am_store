...
// load all icons with index 0 from a dll
...
{
    public static void ExtractAndSaveIcon(string exePath, string saveDir)
    {
        try
        {
            // 加载 exe 文件的图标
            var icons = IconInfo.LoadIconsFromBinary(exePath, 0);

            // 获取 256x256 尺寸的图标
            var icon256 = icons.FirstOrDefault(i => i.WithIcon(icon => icon.Size.Width == 256));

            if (icon256 == null)
            {
                Console.WriteLine("未找到 256x256 尺寸的图标。");
                return;
            }

            // 获取图标并保存为 PNG 格式
            icon256.WithIcon(icon =>
            {
                // 获取图标的 Bitmap（确保透明通道正确）
                using (Bitmap bitmap = icon.ToBitmap())
                {
                    // 构造保存路径
                    string fileName = $"{System.IO.Path.GetFileNameWithoutExtension(exePath)}_256x256.png";
                    string savePath = System.IO.Path.Combine(saveDir, fileName);

                    // 保存图标为 PNG 文件
                    bitmap.Save(savePath, System.Drawing.Imaging.ImageFormat.Png);
                    Console.WriteLine($"图标已保存为 {savePath}");
                }
            });
        }
        catch (Exception ex)
    ...
    static void Main(string[] args)
            {
                if (args.Length != 2)
                {
                    Console.WriteLine("用法: IconExtractorApp.exe <exe文件路径> <保存目录>");
                    return;
                }
                ...
                // 调用图标提取和保存方法
                ExtractAndSaveIcon(exePath, saveDir);
            }
}
